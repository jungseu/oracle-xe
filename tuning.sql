...
REM 실행계획의 단계별 일량 정보 확인을 위해 다음 권한 부여
REM DBMS_XPLAN.DISPLAY_CURSOR 함수를 사용 할 수 있음

REM EXPLAIN PLAN FOR 대신 힌트를 사용
REM SELECT /*+ GATHER_PLAN_STATISTICS */ ~

REM 실행계획 확인
REM SELECT * FROM TABLE(DBMS_XPLAN.DISPLAY_CURSOR(NULL, NULL, 'ALLSTATS LAST -ROWS +PREDICATE +ALIAS +OUTLINE'));
...
REM 고객 테이블(CUSTOMERS) : 1만건의 데이터 저장
REM CUSTNO : 고객번호(1에서 10000까지 고유한 값. 기본키)
REM CNAME : 고객명(랜덤 문자열)
REM PHONE : 전화번호(1000001~1010000)
REM CITY : 거주지(10개 종류. 서울, 부산, 대전, 제주, 광주, 대구, 인천, 성남, 강원, 춘천)
REM GRADE : 등급(3종류, NORMAL, VIP, VVIP)
REM BIRTHDATE : 생일(랜덤)
REM GENDER : 성별(2종류, M, F)
...
REM 상품 테이블(PRODUCTS) : 1000건의 데이터 저장
REM PRODNO : 상품코드(10001에서 11000까지 고유한 값. 기본키)
REM PNAME : 상품명(랜덤 문자열)
REM PRICE : 상품 가격(랜덤 값)
REM PSIZE : 상품 사이즈(5개 종류. S, L, XL, 2XL, 3XL)
REM COLOR : 색상(10개 종류. 'WHITE', 'BLACK', 'RED', 'ORANGE', 'YELLOW', 'GREEN', 'BLUE', 'BROWN', 'PURPLE', 'SILVER')
...
REM 택배 회사 테이블(DELIVERY) : 100건의 테이터 저장
REM CODE : 회사 고유 코드(1에서 10까지 고유한 값. 기본키)
REM NAME : 회사명
REM LOCATION : 주소명
...
REM 주문 테이블(ORDERS) : 10만건의 데이터 저장
REM ORDERNO : 주문번호(1에서 100000까지 고유한 값. 기본키)
REM ADDRESS : 배송지(랜덤 문자열)
REM ORDERDATE : 주문일자(랜덤 날짜)
REM TOTAL : 전체 주문액
REM PAYTYPE : 지불방식(3개 종류. 신용카드, 체크카드, 계좌이체)
REM STATUS : 배송상태(5개 종류. 결제완료, 배송준비중, 배송중, 배송완료, 구매확정)
REM CUSTNO : 고객번호(외래키. CUSTOMERS(CUSTNO) 참조)
REM CODE : 택배회사 코드(외래키. DELIVERY(CODE) 참조)
...
REM 상세 주문(ORDERDETAILS) : 약 20만건의 데이터 저장
REM ORDERSEQ : 일련번호(기본키)
REM ORDERNO : 주문번호(외래키. ORDERS(ORDERNO) 참조)
REM PRODNO : 상품번호(외래키. PRODUCTS(PRODNO) 참조)
REM PRICE : 주문시점의 상품가격
REM QTY : 수량(랜덤값)
REM OPTION : 추가 옵션(랜덤 문자열)
...
REM 외래키에 인덱스 설정
REM CREATE INDEX ORDERS_CUSTNO_IDX ON ORDERS(CUSTNO);
REM CREATE INDEX ORDERS_CODE_IDX ON ORDERS(CODE);
REM CREATE INDEX OD_ORDERNO_IDX ON ORDERDETAILS(ORDERNO);
REM CREATE INDEX OD_PRODNO_IDX ON ORDERDETAILS(PRODNO);
