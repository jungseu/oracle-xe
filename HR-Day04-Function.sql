...
함수 유형
    -단일행 함수-단일행 매게변수를 받아 결과 값 반환 
    -여러행 함수-여러행 매게변수를 받아 결과 값 반환 
*/
/*
단일행  변수
*/
...
--TO_CHAR()함수-날짜 또는 숫자 사용가능
    ...
    HH또는 HH12또는 HH24-하루 시간 또는 반일 시간(1-12)또는 전일시간 (0-23)
    MI-분(0-59)
    ss- 초(0-59)
...
함수 중첩
    ...
    중첩된 함수는 가장 깊은 레벨에서 가장 낮은 레벨로도 평가됩니다
*/
...
/*
NVL2()함수
    첫번째 표현식을 검사합니다. 첫번쨰 표현식이 null이 아니면 NVL2 함수는 두번쨰 표현식을 반환합니다
...
/*
NULLIF() 함수
    두 표현식을 비교 같으면 null반환, 다르면 extra을 반환합니다
    그러나 emtra1에 대해 리터럴 null을 지정할 수 없습니다
*/
SELECT first_name, LENGHTH(first_name) "expr1",
    last_name, LENGHTH(last_name) "expr2",
    NULLIF(LENGHTH(first_name),  LENGHTH(last_name))result
FROM employees;
...
COALESCE(TO_CHAR(communision_pct), TO_CHAR(manager_id),
        'NO communision and no manager')
FROM employees;

/*
조건부 표현식
    SQL문에서 IF-THEN-ELSE 논리를 사용할 수 있습니다
    -case식
    -DECONDE() 함수
*/
/*
